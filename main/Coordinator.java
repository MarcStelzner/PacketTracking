...
public class Coordinator {
	...
	public void run(){
		...
		//TODO: maybe an additional backup-save before getting the Traces ?
	...
	private ArrayList<MultihopPacketTrace> getTraces(ArrayList<MACPacket> packets){
		...
		//timeBetweenTraces is in seconds, TODO: maybe variable by user ? 
			...
			if(tmpFlowLabel >= 0){
				...
				for(MultihopPacketTrace s : traces){
					if(s.getFlowLabel() == tmpFlowLabel){
						...
						if(byteArrayToInt(4,p.getSeconds()) > s.getLastTime()+timeBetweenTraces 
								|| (byteArrayToInt(4,p.getSeconds()) == s.getLastTime()+timeBetweenTraces && byteArrayToInt(4,p.getMilliSeconds()) >= s.getLastTimeMilliseconds()) ){
				...
				for(MultihopPacketTrace s : traces){  
					for(MACPacket sp : s.getPacketList()){
						...
						if(p.getDestinationNode().equals(sp.getDestinationNode())
								...
								&& !(byteArrayToInt(4,p.getSeconds()) > (byteArrayToInt(4,sp.getSeconds())+tmpTimeBetweenStreams))
								&& !((byteArrayToInt(4,p.getSeconds())+tmpTimeBetweenStreams) < byteArrayToInt(4,sp.getSeconds()))){
							found = true;
							break;
						}
					...
					if(checkLaterTag == i){
						...
						for(MACPacket sp : s.getPacketList()){
							...
							if(streamPacketTag == checkLaterTag && p.getDestinationNode().equals(sp.getDestinationNode())
									...
									&& !(byteArrayToInt(4,p.getSeconds()) > (byteArrayToInt(4,sp.getSeconds())+timeBetweenTraces))
									&& !((byteArrayToInt(4,p.getSeconds())+timeBetweenTraces) < byteArrayToInt(4,sp.getSeconds()))){
								found = true;
								s.addPacket(p);
			...
			if(possibleAddress >= 0){
				...
				for(Node n : m.getIntermediateNodes()){
					...
					if(n.getNodeId().length == 2){
						int tmpIntNodeId = byteArrayToInt(2, n.getNodeId());	
						tmpCheckNodeId = tmpIntNodeId % 1024;
					} else {
						long tmpLongNodeId = byteArrayToLong(8, n.getNodeId());
